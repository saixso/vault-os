...
- **vault path** (optional): resolved automatically. Check in order: (1) `vault:` line in `.claude/CLAUDE.md` (written by `/teams-deploy`), (2) `./wiki/` in current directory, (3) ask the user. Do not scan home directory, environment, hooks, or session context.
